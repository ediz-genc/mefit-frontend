...
export class UserService {
  ...
  private currentUser: User = {
    ...
    bio: "Test bio that might be longer in prod. Alot longer in fact, so long that you might need to test it on " +
      "multiple lines. And even then, it might not be enough. Go ahead, try your best. Test bio that might be longer in prod. Alot longer in fact, so long that you might need to test it on " +
      "multiple lines. And even then, it might not be enough. Go ahead, try your best. Test bio that might be longer in prod. Alot longer in fact, so long that you might need to test it on " +
      "multiple lines. And even then, it might not be enough. Go ahead, try your best. Test bio that might be longer in prod. Alot longer in fact, so long that you might need to test it on " +
      "multiple lines. And even then, it might not be enough. Go ahead, try your best. Test bio that might be longer in prod. Alot longer in fact, so long that you might need to test it on " +
      "multiple lines. And even then, it might not be enough. Go ahead, try your best. Test bio that might be longer in prod. Alot longer in fact, so long that you might need to test it on " +
  ...
  updateProfile(id: String, height: number, weight: number, bio: string): Observable<User> {
    // Make an HTTP PUT request to update the user's profile data
    const body = { height, weight, bio };
    return this.http.put<User>(`${this.baseApiUrl}/users/${id}`, body);
  }

  // Add a method to update the user's profile picture
  updateProfilePicture(id: String, file: File): Observable<User> {
    // Create a FormData object and append the profile picture file to it
    const formData = new FormData();
    formData.append('profilePicture', file, file.name);

    // Make an HTTP POST request to update the user's profile picture
    return this.http.post<User>(`${this.baseApiUrl}/users/${id}/profile-picture`, formData);
  }
